...
describe('ResponsePaneActions', () => {
  ...
  it('copies sanitized debug Markdown from the more-actions menu', async () => {
    render(
    ...
    expect(copiedText).not.toContain('Bearer test-token');
  });

  it('renders a neutral debug Markdown copy action', () => {
    render(
      <ThemeProvider theme={theme}>
        <ResponsePaneActions
          item={item}
    ...
    expect(screen.queryByRole('menuitem', { name: 'Copy for AI' })).not.toBeInTheDocument();
  });
});
