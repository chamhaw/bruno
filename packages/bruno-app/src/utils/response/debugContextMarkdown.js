...
const SENSITIVE_KEY_PATTERN = /authorization|proxy-authorization|cookie|set-cookie|api[-_]?key|access[-_]?token|refresh[-_]?token|id[-_]?token|token|secret|password|passwd|private[-_]?key|client[-_]?secret|session/i;

const isSensitiveKey = (name) => SENSITIVE_KEY_PATTERN.test(String(name));
...
const redactHeaderValue = (name, value) => {
...
const redactUrl = (url) => {
  if (!url) return url;
  const stringValue = String(url);

  try {
    const hasScheme = /^[a-z][a-z\d+\-.]*:\/\//i.test(stringValue);
    const parsed = new URL(stringValue, 'http://bruno.local');
    ...
    return stringValue.replace(/([?&][^=&]*(?:authorization|cookie|api[-_]?key|access[-_]?token|refresh[-_]?token|id[-_]?token|token|secret|password|passwd|private[-_]?key|client[-_]?secret|session)[^=&]*=)[^&#\s]*/gi, `$1${REDACTED_VALUE}`);
  }
};
...
const redactText = (text) => {
  if (!text) return text;
  return String(text)
    .replace(/((?:authorization|proxy-authorization|cookie|set-cookie|api[-_]?key|access[-_]?token|refresh[-_]?token|id[-_]?token|token|secret|password|passwd|private[-_]?key|client[-_]?secret|session)\s*[:=]\s*)([^\r\n&]+)/gi, `$1${REDACTED_VALUE}`)
    .replace(/([?&][^=&]*(?:authorization|cookie|api[-_]?key|access[-_]?token|refresh[-_]?token|id[-_]?token|token|secret|password|passwd|private[-_]?key|client[-_]?secret|session)[^=&]*=)[^&#\s]*/gi, `$1${REDACTED_VALUE}`);
};

const redactStructuredValue = (value, seen = new WeakSet()) => {
  ...
  return Object.entries(value).reduce((acc, [key, nestedValue]) => {
    acc[key] = isSensitiveKey(key) ? REDACTED_VALUE : redactStructuredValue(nestedValue, seen);
    return acc;
  }, {});
};
...
const normalizeHeaders = (headers) => {
  ...
  if (Array.isArray(headers)) {
    return headers
      ...
      .map(({ name, value }) => ({ name: String(name), value: redactHeaderValue(name, value) }));
  }
...
const stringifyBody = (body, options = {}) => {
  ...
  if (typeof body === 'string') {
    ...
    if (parsedBody && typeof parsedBody === 'object') {
      return truncateText(JSON.stringify(redactStructuredValue(parsedBody), null, 2), maxChars);
    }
    return truncateText(redactText(body), maxChars);
  }

  try {
    return truncateText(JSON.stringify(redactStructuredValue(body), null, 2), maxChars);
  } catch {
    return truncateText(redactText(String(body)), maxChars);
  }
};
...
const buildCurl = (request) => {
  ...
  const segments = ['curl', '-X', method, quoteShellArg(redactUrl(request.url))];
...
const parseStructuredFallback = ({ request, response, timeline }) => {
  ...
  return {
    request: {
      method: request?.method || fallbackRequest.method,
      url: request?.url || fallbackRequest.url,
      headers: request?.headers || headerLinesToObject(parsedTimeline.requestHeaders),
      data: request?.data ?? tryParseStructuredText(parsedTimeline.requestBodyLines.join('\n'))
    },
    response: {
...
const formatContextSection = ({ selectedRequest, request, response, collection, item }) => {
  ...
  if (request?.url) {
    lines.push(`- URL: ${redactUrl(request.url)}`);
  }
